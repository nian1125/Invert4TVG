...
class LazySupervisedDataset(Dataset):
    ...
    def process_video(self, video_file):
        ...
        frames, _, metadata = torchvision.io.read_video(
            video_file, pts_unit="sec", output_format="TCHW"
        )
        total_frames = len(frames)
        avg_fps = metadata["video_fps"]
...
def preprocess_qwen_2_visual_for_grpo_trainer(
    sources,
    tokenizer: transformers.PreTrainedTokenizer,
    grid_thw: List = [],
    visual_type: str = "image",
    ...
    visual_replicate_index = 0
    input_ids, targets, prompts, prompts_text, answers = [], [], [], [], []
...
class LazyGRPODataset(LazySupervisedDataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            sources = copy.deepcopy([e["conversations"] for e in sources])
            data_dict = preprocess_qwen_2_visual_for_grpo_trainer(
                sources, self.tokenizer, grid_thw=grid_thw_merged, visual_type="image"
            )
            position_ids, _ = self.get_rope_index(
                self.data_args.image_processor.merge_size,
        ...
        if isinstance(i, int):
            data_dict = dict(
                ...
                prompts_text=data_dict["prompts_text"][0],
                answers=data_dict["answers"][0],
            )
...
class DataCollatorForGRPODataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, position_ids, prompts, prompts_text, answers = tuple(
            [instance[key] for instance in instances]
            for key in ("input_ids", "labels", "position_ids", "prompts", "prompts_text", "answers")
        )
        input_ids = torch.nn.utils.rnn.pad_sequence(
            input_ids, batch_first=True, padding_value=self.tokenizer.pad_token_id
        )
...
def make_grpo_data_module(
    ...
    """Make dataset and collator for supervised fine-tuning."""
    train_dataset = LazyGRPODataset(tokenizer=tokenizer, data_args=data_args)
    if data_args.data_flatten:
        raise ValueError("GRPO does not support data_flatten")
